<p align="center">
  <img src="wishcraft/assets/magical_lamp.png" alt="WishCraft" width="120">
</p>
...
<p align="center">
  <img src="architecture-diagram.png" alt="WishCraft System Architecture" width="100%">
</p>
...
| 🧠 **10 Built-in Skills** | Gmail, Excel, Numbers, Mail, PPTX, file organizer, image tools, and more |
...
- *"Open Gmail, find the invoice from Amazon, download the PDF, then create an Excel spreadsheet summarizing all charges"*
- *"Go to my Fiverr dashboard, check pending orders, screenshot each one, and draft a status update email in Gmail"*
- *"Research the top 5 competitors for my SaaS product using Google, compile findings into a 10-slide presentation with charts"*
- *"Open VS Code, create a new Python project, write unit tests for the calculator module, and run them in Terminal"*
- *"Check my unread WhatsApp messages, summarize them, create a reminder in Notes for anything urgent, and reply to each one"*
- *"Open Chrome, log into my bank dashboard, screenshot the monthly statement, then organize my Downloads folder by date"*
- *"Write a 2-page cover letter for a software engineer role, save it to Word, then open Gmail and draft an application email with the file attached"*
- *"Take a screenshot of my desktop, analyze what apps are open, close unused ones, set volume to 30%, and enable dark mode"*
...
│   ├── skills/                     # 10 discoverable skill modules
...
│   │   ├── commands.py             # ~90 universal cross-platform commands (Gmail, WhatsApp, system, files)
│   │   ├── macos_automation.py     # ~286 macOS functions (AppleScript + PyAutoGUI) — blind, no vision
│   │   ├── windows_automation.py   # ~49 Windows functions (PowerShell + PyAutoGUI) — blind, no vision
│   │   ├── linux_automation.py     # ~55 Linux functions (xdotool + PyAutoGUI) — blind, no vision
│   │   └── computer_use_agent.py   # Gemini Computer Use visual agent (screenshots + AI)
│   └── renderer/
...
This project is licensed under the **MIT License** — see the [LICENSE](wishcraft/LICENSE) file for details.
...
- 📧 **Email:** [mdabubokkor@icloud.com](mailto:mdabubokkor@icloud.com)
...
Member profile: [https://gdg.community.dev/u/mr7vuz/#/about](https://gdg.community.dev/u/mr7vuz/#/about)

### 🚀 Automated Cloud Deployment

Deployment is fully automated via [`deploy.sh`](wishcraft-server/deploy.sh) — a single script that:

1. Creates a Cloud Storage bucket for encrypted sync
2. Enables required GCP APIs (Cloud Run, Firestore, Cloud Storage)
3. Builds and deploys to Cloud Run from source
4. Outputs the live service URL
...
This project was built for the **Gemini API Developer Competition — Live Agent Challenge**. Built with Google Gemini models (Live API, Computer Use, Flash) and deployed on Google Cloud.
